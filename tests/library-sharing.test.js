...
test('library for everyone: own library, reader/editor/manager, selection and revocation', async () => {
  const pg = new PGlite();
  ...
  try {
    ...
    await pg.query("select biblioteca_invitar('ILEANA@x.com','manager')");
    ...
    // Cuenta no invitada: biblioteca propia aislada.
    await as(extrana);
    ...
    // Reader: puede seleccionar y ver, pero no modificar ni administrar.
    await as(amiga);
    assert.deepEqual(await books(), []);
    ...
    assert.equal((await pg.query('select biblioteca_can_manage() as m')).rows[0].m, false);
    await assert.rejects(pg.query("select biblioteca_invitar('extrana@x.com','reader')"), /No tenés permiso/);

    // Manager: puede editar la biblioteca de mamá y gestionar accesos sin volverse dueña.
    await as(ileana);
    await pg.query('select biblioteca_elegir($1)', [mama]);
    assert.equal((await pg.query('select biblioteca_can_write() as w')).rows[0].w, true);
    assert.equal((await pg.query('select biblioteca_can_manage() as m')).rows[0].m, true);
    assert.equal((await pg.query("update biblioteca_libros set paginas=120 returning id")).rows.length, 1);
    await pg.query("select biblioteca_invitar('extrana@x.com','editor')");
    assert.equal((await pg.query("select count(*)::int as n from biblioteca_miembros() where email='extrana@x.com'")).rows[0].n,1);

    // El nuevo editor puede trabajar sobre mamá y conserva su biblioteca personal intacta.
    await as(extrana);
    assert.deepEqual(await books(), ['Libro de la extraña']);
    await pg.query('select biblioteca_elegir($1)', [mama]);
    assert.deepEqual(await books(), ['Libro de mamá']);
    assert.equal((await pg.query("update biblioteca_libros set titulo='Libro editado' returning id")).rows.length,1);
    await pg.query('select biblioteca_elegir($1)', [extrana]);
    assert.deepEqual(await books(), ['Libro de la extraña']);

    // Revocar acceso devuelve a la biblioteca propia y no borra datos personales.
    await as(mama);
    ...
    // Una invitada puede salir sola; su propia biblioteca queda separada.
    await as(ileana);
