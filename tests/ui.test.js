...
test('demo UI preserves navigation, diary, incremental water, habit fields and calendar',async()=>{
 ...
 try{w.eval(bundle.outputFiles[0].text);const click=async selector=>{const el=w.document.querySelector(selector);assert.ok(el,selector);el.click();await new Promise(r=>setTimeout(r,35));};
 ...
 await click('[data-action=journal]');w.document.querySelector('[name=text]').value='Mi diario privado';w.document.querySelector('#modal form').dispatchEvent(new w.Event('submit',{bubbles:true,cancelable:true}));await new Promise(r=>setTimeout(r,35));assert.match(w.document.body.textContent,/Mi diario privado/);
