...
async function waitFor(check,{timeout=1800,step=20}={}){const start=Date.now();while(Date.now()-start<timeout){if(check())return;await sleep(step);}assert.ok(check(),'La interfaz no alcanzó el estado esperado dentro del tiempo de prueba.');}

test('demo UI preserves navigation, diary, unified water, habit fields and calendar',async()=>{
 ...
 try{w.eval(bundle.outputFiles[0].text);const click=async selector=>{const el=w.document.querySelector(selector);assert.ok(el,selector);el.click();await sleep(20);};
 ...
 assert.ok(w.document.querySelector('[data-view=progress]'),'Progreso debe ser una vista principal');
 await click('[data-action=water-add][data-ml="250"]');await click('[data-action=water-add][data-ml="500"]');await waitFor(()=>/0[,.]75 L/.test(w.document.body.textContent));assert.match(w.document.body.textContent,/0[,.]75 L/);
 await click('[data-action=water-remove]');await waitFor(()=>/0[,.]5 L/.test(w.document.body.textContent));assert.match(w.document.body.textContent,/0[,.]5 L/);
 ...
 await click('[data-view=progress]');assert.match(w.document.body.textContent,/PROMEDIO DE AGUA/);
