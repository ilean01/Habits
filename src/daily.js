...
export function dayWelcome(hour=new Date().getHours()) {
 if(hour<5)return {greeting:'Buenas noches',icon:'Moon',subtitle:'Ya es tarde. Descansá, mañana seguimos a tu ritmo.'};
 if(hour<12)return {greeting:'Buen día',icon:'Sun',subtitle:'Un nuevo día para dedicarte un poco de tiempo.'};
 if(hour<19)return {greeting:'Buenas tardes',icon:'Sun',subtitle:'Todavía hay espacio para algo que te haga bien.'};
 return {greeting:'Buenas noches',icon:'Moon',subtitle:'Un momento para descansar y reconocer lo que hiciste hoy.'};
}
