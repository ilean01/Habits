import {supabase,currentUser} from './client.js';
...
export async function insertBook(data){const {data:row,error}=await supabase.from(T.books).insert({...data,owner_id:currentUser.id}).select().single();if(error)throw error;return row}
...
export async function insertReading(data){const {data:row,error}=await supabase.from(T.readings).insert({...data,owner_id:currentUser.id}).select().single();if(error)throw error;return row}
export async function insertFinished(data){const {data:row,error}=await supabase.from(T.finished).insert({...data,owner_id:currentUser.id}).select().single();if(error)throw error;return row}
export async function upsertPerson(name,extra={}){const n=String(name||'').trim();if(!n)return null;const {data,error}=await supabase.from(T.people).upsert({owner_id:currentUser.id,nombre:n,...extra},{onConflict:'owner_id,nombre'}).select().single();if(error)throw error;return data}
export async function insertLoan(data){const {data:row,error}=await supabase.from(T.loans).insert({...data,owner_id:currentUser.id}).select().single();if(error)throw error;return row}
...
export async function saveConfig(values){const rows=Object.entries(values).map(([clave,valor])=>({owner_id:currentUser.id,clave,valor:String(valor??'')}));const {error}=await supabase.from(T.config).upsert(rows,{onConflict:'owner_id,clave'});if(error)throw error}
