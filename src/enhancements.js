...
import {dayKey,weeklyProgress,flexibleWeekly} from './domain.js';
import {holidayOn} from './paraguay-holidays.js';
...
function installStyles(){
 ...
 .weekly-frequency{grid-column:1/-1;border:1px solid var(--line);border-radius:10px;padding:14px;margin:8px 0}.weekly-frequency legend{font-size:13px;font-weight:650}.weekly-frequency-grid{display:grid;grid-template-columns:1fr 150px;gap:10px}.weekly-help,.weekly-goal{font-size:12px;color:var(--muted);line-height:1.4}.weekly-help{margin:8px 0 0}.weekly-goal{margin:-5px 0 10px}.weekly-goal progress{margin:5px 0}.weekly-complete{box-shadow:inset 0 0 0 1px #9db38f}
 .work-focus{border:1px solid #d9e3eb;background:#f2f6f8;border-radius:11px;padding:12px 15px;margin-bottom:14px;font-size:13px;color:#557080}.work-priority-card{order:-10}
 .install-banner{border:1px solid #dce3d4;background:#f1f5ed;border-radius:13px;padding:17px;margin-bottom:20px}.install-banner h3{font-family:var(--serif);font-size:20px;margin-bottom:8px}.install-steps{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.install-step{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:11px;text-align:center;font-size:12px}.install-step b{display:block;font-size:22px;margin-bottom:5px}.install-close{float:right;min-width:44px;min-height:44px}.install-help.enhanced-install .install-steps{margin:12px 0}
 .holiday-today{border-left:3px solid #b79254;padding-left:10px}
 @media(max-width:650px){.weekly-frequency-grid,.install-steps{grid-template-columns:1fr}.install-banner{padding:14px}}
...
function prioritizeWork(){
 ...
 const banner=document.createElement('div');banner.className='work-focus';banner.innerHTML=`💼 Estás dentro de <strong>${esc(block.name||'tu horario de trabajo')}</strong>${block.end?` hasta las ${esc(block.end)}`:''}. Primero aparece lo laboral; lo personal sigue disponible.`;grid.closest('section')?.insertBefore(banner,grid);
...
function decorateHolidayToday(){const h=holidayOn(dayKey());if(!h)return;const hero=document.querySelector('.day-hero');if(!hero||document.querySelector('.holiday-today'))return;const note=document.createElement('p');note.className='holiday-today';note.textContent=`🇵🇾 Hoy es feriado: ${h.name}`;hero.querySelector('div')?.append(note);}
