...
export function progressView({records,catalog,prettyDate,esc,icon,btn,settings}){
 const today=dayKey(),week=weekKeys(today),allLogs=records('log'),doneLogs=allLogs.filter(l=>week.includes(l.date)&&l.status==='done'),cfg=settings()||{};
 ...
 const water=waterStats(hydrationLogs(allLogs),today,7),restDates=Object.entries(cfg.dayModes||{}).filter(([,mode])=>mode==='descanso').map(([d])=>d),restWeekdays=Object.entries(cfg.weekModes||{}).filter(([,mode])=>mode==='descanso').map(([d])=>Number(d));
 const streakRows=records('habit').filter(h=>!h.archived).map(h=>{
  const value=streak(h,allLogs,today,{restDates,restWeekdays}),unit=flexibleWeekly(h)?(value===1?'semana':'semanas'):(value===1?'día':'días');
 ...
 <section class="panel"><h2>Pequeñas constancias</h2><p class="muted">Los hábitos diarios muestran rachas en días. Las metas flexibles muestran rachas en semanas. Los días configurados como descanso no rompen la racha.</p>${streakRows}</section>
