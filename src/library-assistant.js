...
export function mountLibraryAssistant(host,{getSnapshot}={}){
  ...
  panel.querySelector('form').onsubmit=async event=>{
    ...
    input.value='';addMessage(messages,'user',message);send.disabled=true;panel.setAttribute('aria-busy','true');
    try{
      const snapshot=await loadSnapshot();
      const count=(snapshot.books||[]).filter(b=>!b.eliminado&&b.lista!=='deseos').length;
      if(!panel.querySelector('[data-cloud-mode]').checked){addMessage(messages,'assistant',answerLocally(message,snapshot),`Índice local · ${count.toLocaleString('es-PY')} libros`);return;}
      try{
